...
|Defines what level of session security is used by LDAP (sign, seal, or none). `CVS-Performance` is supporting LDAP signing, if requested by Active Directory. Service type `CVS` does not support LDAP signing. Sealing isn't supported by all service types.
|None
|No
